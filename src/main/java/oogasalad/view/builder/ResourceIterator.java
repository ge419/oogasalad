...
/**
 * Interface that holds some common methods for manipulating resource files and
 * creating interactable items out of them.
 *
 * @author tmh85
 * @author jf295
 */
public interface ResourceIterator {
   ...
   * @param operation lambda expression of your choosing you want to perform on
   *                  each new key entry. Provided argument to the lambda is the
   *                  current key for manipulation.
   */
  ...
  default void runMethodFromString(String method) {
    try {
      this.getClass().getDeclaredMethod(method)
          .invoke(this);
    } catch (Exception ex) {
      ex.printStackTrace();
