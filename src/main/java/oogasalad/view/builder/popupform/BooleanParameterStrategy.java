...
/**
 * A strategy used by the popup form to display a form element when a boolean is required from the
 * user via user input. This consists of one labeled checkbox for the user to input true or false.
 * Example Usage: VBox form = new VBox(); BooleanParameterStrategy strategy = new
 * BooleanParameterStrategy(myBooleanAttribute, myBooleanMetadata);
...
public class BooleanParameterStrategy implements ParameterStrategy, BuilderUtility {
  ...
  /**
   * Creates an instance of BooleanParameterStrategy Can be used to display form data to a user for
   * a bool, validate the input, save to an attribute, and access the corresponding BooleanAttribute
   * and BooleanMetadata
   *
