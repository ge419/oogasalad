...
/**
 * A strategy used by the popup form to display a form element when an integer is required from the
 * user via user input. This consists of a labeled integer spinner. Example Usage: VBox form = new
 * VBox(); IntegerParameterStrategy strategy = new IntegerParameterStrategy(myIntegerAttribute,
 * myIntegerMetadata); form.getChildren().add(strategy.renderInput(myResourceBundle, form));
 *
...
public class IntegerParameterStrategy implements ParameterStrategy, BuilderUtility {
  ...
  /**
   * Creates an instance of IntegerParameterStrategy Can be used to display form data to a user for
   * an int, validate the input, save to an attribute, and access the corresponding IntegerAttribute
   * and IntegerMetadata
   *
