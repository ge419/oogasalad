...
/**
 * Creates a form for user input for any game construct. Uses the attributes of the constructable
 * object to select a strategy for displaying each form element and uses the schema's metadata to
 * validate data. Submitting the form saves the form data to the game construct in-place. Example
 * usage: The example code adds a form for a tile to the root VBox VBox root = new VBox(); new
 * PopupForm(new Tile(), myResourceBundle, root); Scene myScene = new Scene(root); Stage myStage =
 * new Stage(); myStage.setScene(myScene); myStage.show();
 *
