...
/**
 * A strategy used by the popup form to display a form element when a Tile list is required from the
 * user via user input. This consists of one button that creates an event listener when clicked. The
 * event listener allows the user to select tiles to be added to a list of tiles Example Usage: VBox
 * form = new VBox(); TileListParameterStrategy strategy = new
 * TileListParameterStrategy(myTileListAttribute, myTileListMetadata);
...
public class TileListParameterStrategy implements ParameterStrategy, BuilderUtility {
  ...
  /**
   * Creates an instance of TileListParameterStrategy Can be used to display form data to a user for
   * a list of tiles, validate the input, save to an attribute, and access the corresponding
   * TileListAttribute and TileListMetadata
   *
  ...
  private void addHandlerToRoot(ResourceBundle resourceBundle) {
    root.addEventHandler(TileEvent.SELECT_TILE, event -> {
      ...
      element.setText(String.format("%s: %s", resourceBundle.getString("Selected"),
          tiles.stream().map(Tile::getId).collect(Collectors.joining(","))));
    });
  }
  ...
  public Node renderInput(ResourceBundle resourceBundle, Pane form, String objectId) {
    ...
    if (this.attr.getTileIds().isEmpty()) {
      ...
      element = new Button(String.format("%s: %s", resourceBundle.getString("Selected"),
          this.attr.getTileIds().stream().collect(Collectors.joining(","))));
    }
