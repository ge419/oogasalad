...
/**
 * A strategy used by the popup form to display a form element when a Tile is required from the user
 * via user input. This consists of one button that creates an event listener when clicked. The
 * event listener allows the user to select a tile Example Usage: VBox form = new VBox();
...
public class TileParameterStrategy implements ParameterStrategy, BuilderUtility {
  ...
  /**
   * Creates an instance of TileParameterStrategy Can be used to display form data to a user for a
   * tile, validate the input, save to an attribute, and access the corresponding TileAttribute and
   * TileMetadata
   *
  ...
  private void addHandlerToRoot(ResourceBundle resourceBundle) {
    root.addEventHandler(TileEvent.SELECT_TILE, event -> {
      ...
      element.setText(String.format(
          String.format("%s: %s", resourceBundle.getString("Selected"), tile.getId())));
    });
  }
  ...
  public Node renderInput(ResourceBundle resourceBundle, Pane form, String objectId) {
    ...
    if (this.attr.getId() == null) {
      ...
      element = new Button(
          String.format("%s: %s", resourceBundle.getString("Selected"), this.attr.getId()));
    }
